-- Generated by Oracle SQL Developer Data Modeler 23.1.0.087.0806
--   at:        2024-02-20 22:35:20 EST
--   site:      Oracle Database 11g
--   type:      Oracle Database 11g



-- predefined type, no DDL - MDSYS.SDO_GEOMETRY

-- predefined type, no DDL - XMLTYPE

CREATE TABLE billet (
    id_billet                  NUMBER NOT NULL,
    classe                     VARCHAR2(32 CHAR) NOT NULL,
    siege                      VARCHAR2(8 CHAR) NOT NULL,
    transaction_id_transaction NUMBER NOT NULL,
    voyage_id_voyage           NUMBER NOT NULL,
    utilisateur_id_utilisateur NUMBER NOT NULL
);

CREATE UNIQUE INDEX billet__idx ON
    billet (
        transaction_id_transaction
    ASC );

ALTER TABLE billet ADD CONSTRAINT billet_pk PRIMARY KEY ( id_billet );

CREATE TABLE droit (
    id_droit    NUMBER NOT NULL,
    nom         VARCHAR2(64 CHAR) NOT NULL,
    description VARCHAR2(512 CHAR)
);

ALTER TABLE droit ADD CONSTRAINT droit_pk PRIMARY KEY ( id_droit );

CREATE TABLE planete (
    id_planete  NUMBER NOT NULL,
    nom         VARCHAR2(128) NOT NULL,
    type        VARCHAR2(128) NOT NULL,
    gravite     NUMBER NOT NULL,
    description VARCHAR2(1024)
);

ALTER TABLE planete ADD CONSTRAINT planete_pk PRIMARY KEY ( id_planete );

CREATE TABLE rabais (
    id_rabais   NUMBER NOT NULL,
    pourcentage NUMBER NOT NULL,
    date_debut  DATE,
    date_fin    DATE,
    code        VARCHAR2(64 CHAR) NOT NULL
);

ALTER TABLE rabais ADD CONSTRAINT rabais_pk PRIMARY KEY ( id_rabais );

CREATE TABLE session_util (
    session_util_id            NUMBER NOT NULL,
    token                      VARCHAR2(64 CHAR) NOT NULL,
    date_creation              DATE NOT NULL,
    date_expiration            DATE,
    utilisateur_id_utilisateur NUMBER
);

ALTER TABLE session_util ADD CONSTRAINT session_util_pk PRIMARY KEY ( session_util_id );

ALTER TABLE session_util ADD CONSTRAINT session_util__un UNIQUE ( token );

CREATE TABLE transaction (
    id_transaction   NUMBER NOT NULL,
    "date"           DATE NOT NULL,
    prix_total       NUMBER NOT NULL,
    billet_id_billet NUMBER NOT NULL
);

CREATE UNIQUE INDEX transaction__idx ON
    transaction (
        billet_id_billet
    ASC );

ALTER TABLE transaction ADD CONSTRAINT transaction_pk PRIMARY KEY ( id_transaction );

CREATE TABLE utilisateur (
    id_utilisateur     NUMBER NOT NULL,
    email              VARCHAR2(256 CHAR) NOT NULL,
    mot_de_passe       VARCHAR2(256 CHAR) NOT NULL,
    nom                VARCHAR2(64 CHAR) NOT NULL,
    prenom             VARCHAR2(64 CHAR) NOT NULL,
    adresse            VARCHAR2(128),
    telephone          NUMBER,
    planete_residence  VARCHAR2(128 CHAR),
    planete_id_planete NUMBER NOT NULL
);

ALTER TABLE utilisateur ADD CONSTRAINT utilisateur_pk PRIMARY KEY ( id_utilisateur );

CREATE TABLE utilisateur_droit (
    utilisateur_id_utilisateur NUMBER NOT NULL,
    droit_id_droit             NUMBER NOT NULL
);

ALTER TABLE utilisateur_droit ADD CONSTRAINT utilisateur_droit_pk PRIMARY KEY ( utilisateur_id_utilisateur,
                                                                                droit_id_droit );

CREATE TABLE vaisseau (
    id_vaisseau        NUMBER NOT NULL,
    nom                VARCHAR2(128 CHAR) NOT NULL,
    type_de_propulsion VARCHAR2(128 CHAR) NOT NULL,
    capacite           NUMBER NOT NULL
);

ALTER TABLE vaisseau ADD CONSTRAINT vaisseau_pk PRIMARY KEY ( id_vaisseau );

CREATE TABLE voyage (
    id_voyage            NUMBER NOT NULL,
    origine              VARCHAR2(128 CHAR) NOT NULL,
    destination          VARCHAR2(128 CHAR) NOT NULL,
    date_depart          DATE NOT NULL,
    duree                NUMBER NOT NULL,
    prix                 NUMBER,
    vaisseau_id_vaisseau NUMBER NOT NULL,
    planete_id_planete2  NUMBER NOT NULL,
    planete_id_planete   NUMBER NOT NULL,
    rabais_id_rabais     NUMBER
);

ALTER TABLE voyage ADD CONSTRAINT voyage_pk PRIMARY KEY ( id_voyage );

CREATE TABLE wishlist (
    id_wishlist                NUMBER NOT NULL,
    date_ajout                 DATE NOT NULL,
    utilisateur_id_utilisateur NUMBER
);

ALTER TABLE wishlist ADD CONSTRAINT wishlist_pk PRIMARY KEY ( id_wishlist );

CREATE TABLE wishlist_voyage (
    wishlist_id_wishlist NUMBER NOT NULL,
    voyage_id_voyage     NUMBER NOT NULL
);

ALTER TABLE wishlist_voyage ADD CONSTRAINT wishlist_voyage_pk PRIMARY KEY ( wishlist_id_wishlist,
                                                                            voyage_id_voyage );

ALTER TABLE billet
    ADD CONSTRAINT billet_transaction_fk FOREIGN KEY ( transaction_id_transaction )
        REFERENCES transaction ( id_transaction );

ALTER TABLE billet
    ADD CONSTRAINT billet_utilisateur_fk FOREIGN KEY ( utilisateur_id_utilisateur )
        REFERENCES utilisateur ( id_utilisateur );

ALTER TABLE billet
    ADD CONSTRAINT billet_voyage_fk FOREIGN KEY ( voyage_id_voyage )
        REFERENCES voyage ( id_voyage );

ALTER TABLE session_util
    ADD CONSTRAINT session_util_utilisateur_fk FOREIGN KEY ( utilisateur_id_utilisateur )
        REFERENCES utilisateur ( id_utilisateur );

ALTER TABLE transaction
    ADD CONSTRAINT transaction_billet_fk FOREIGN KEY ( billet_id_billet )
        REFERENCES billet ( id_billet );

ALTER TABLE utilisateur_droit
    ADD CONSTRAINT util_droit_util_fk FOREIGN KEY ( utilisateur_id_utilisateur )
        REFERENCES utilisateur ( id_utilisateur );

ALTER TABLE utilisateur_droit
    ADD CONSTRAINT utilisateur_droit_droit_fk FOREIGN KEY ( droit_id_droit )
        REFERENCES droit ( id_droit );

ALTER TABLE utilisateur
    ADD CONSTRAINT utilisateur_planete_fk FOREIGN KEY ( planete_id_planete )
        REFERENCES planete ( id_planete );

ALTER TABLE voyage
    ADD CONSTRAINT voyage_planete_fk FOREIGN KEY ( planete_id_planete )
        REFERENCES planete ( id_planete );

ALTER TABLE voyage
    ADD CONSTRAINT voyage_planete_fkv2 FOREIGN KEY ( planete_id_planete2 )
        REFERENCES planete ( id_planete );

ALTER TABLE voyage
    ADD CONSTRAINT voyage_rabais_fk FOREIGN KEY ( rabais_id_rabais )
        REFERENCES rabais ( id_rabais );

ALTER TABLE voyage
    ADD CONSTRAINT voyage_vaisseau_fk FOREIGN KEY ( vaisseau_id_vaisseau )
        REFERENCES vaisseau ( id_vaisseau );

ALTER TABLE wishlist
    ADD CONSTRAINT wishlist_utilisateur_fk FOREIGN KEY ( utilisateur_id_utilisateur )
        REFERENCES utilisateur ( id_utilisateur );

ALTER TABLE wishlist_voyage
    ADD CONSTRAINT wishlist_voyage_voyage_fk FOREIGN KEY ( voyage_id_voyage )
        REFERENCES voyage ( id_voyage );

ALTER TABLE wishlist_voyage
    ADD CONSTRAINT wishlist_voyage_wishlist_fk FOREIGN KEY ( wishlist_id_wishlist )
        REFERENCES wishlist ( id_wishlist );



-- Oracle SQL Developer Data Modeler Summary Report: 
-- 
-- CREATE TABLE                            12
-- CREATE INDEX                             2
-- ALTER TABLE                             28
-- CREATE VIEW                              0
-- ALTER VIEW                               0
-- CREATE PACKAGE                           0
-- CREATE PACKAGE BODY                      0
-- CREATE PROCEDURE                         0
-- CREATE FUNCTION                          0
-- CREATE TRIGGER                           0
-- ALTER TRIGGER                            0
-- CREATE COLLECTION TYPE                   0
-- CREATE STRUCTURED TYPE                   0
-- CREATE STRUCTURED TYPE BODY              0
-- CREATE CLUSTER                           0
-- CREATE CONTEXT                           0
-- CREATE DATABASE                          0
-- CREATE DIMENSION                         0
-- CREATE DIRECTORY                         0
-- CREATE DISK GROUP                        0
-- CREATE ROLE                              0
-- CREATE ROLLBACK SEGMENT                  0
-- CREATE SEQUENCE                          0
-- CREATE MATERIALIZED VIEW                 0
-- CREATE MATERIALIZED VIEW LOG             0
-- CREATE SYNONYM                           0
-- CREATE TABLESPACE                        0
-- CREATE USER                              0
-- 
-- DROP TABLESPACE                          0
-- DROP DATABASE                            0
-- 
-- REDACTION POLICY                         0
-- 
-- ORDS DROP SCHEMA                         0
-- ORDS ENABLE SCHEMA                       0
-- ORDS ENABLE OBJECT                       0
-- 
-- ERRORS                                   0
-- WARNINGS                                 0
