                ...
                if (resultUser.rows.length > 0) {
                    ...
                    if (mdp_valide) {
                        ...
                        if (planeteResult.rows.length > 0) {
                            ...
                            return res.render('pages/connexion', {  message_positif: 'Connexion au compte effectuée avec succès!',
                                                                    message_negatif: "Aucune planète liée à l'utilisateur.",
                        ...
                        return res.render('pages/connexion', { message_negatif: 'Mot de passe incorrect.', est_connecte: req.session.est_connecte, est_admin: req.session.est_admin });
                    }
                    ...
                    if (mdp_valide) {
                        ...
                        return res.render('pages/', { message_positif: 'Connexion au compte admin effectuée avec succès!', est_connecte: req.session.est_connecte, est_admin: req.session.est_admin });
                    } else {
                        // Le mot de passe est incorrect
                        return res.render('pages/connexion', { message_negatif: 'Mot de passe incorrect.', est_connecte: req.session.est_connecte, est_admin: req.session.est_admin });
                    }
                    ...
                    return res.render('pages/connexion', { message_negatif: "L'utilisateur n'existe pas.", est_connecte: req.session.est_connecte, est_admin: req.session.est_admin });
                }
            } catch (err) {
                console.error(err);
                return res.render('pages/connexion', { message_negatif: 'Erreur lors de la connexion à la base de données.', est_connecte: req.session.est_connecte, est_admin: req.session.est_admin });
            } finally {
                if (connexion) {
                    await connexion.close();
                }
        ...
        */
        .post([
            ...
            try {
                ...
                console.error(err);
                return res.render('pages/inscription', { message_negatif: 'Erreur lors de la connexion à la base de données', est_connecte: req.session.est_connecte, est_admin: req.session.est_admin });
            }
        });
